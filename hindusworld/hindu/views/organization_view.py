...
class OrgnizationView(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        queryset = self.filter_queryset(self.get_queryset().filter(status='SUCCESS'))
    ...
    def retrieve(self, request, *args, **kwargs):
        # Retrieve the object based on the pk or custom filtering
        instance = self.get_object()
        
        # Add any custom logic here, e.g., status check or additional filters
...
class AdminOrganizationViewSet(viewsets.ModelViewSet):
    ...
    # ⛳ Filter by status (pending / success)
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Step 1: Normal update (other fields)
        serializer = self.get_serializer(instance, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()

        # Step 2: Handle org_images update
        org_images = request.data.get("org_images")
        if org_images and org_images != "null":
            # Upload new image to Azure
            new_image = save_image_to_azure(
                org_images,
                instance._id,
            ...
            if new_image:
                ...
                if instance.org_images:
                    instance.org_images = instance.org_images + "," + new_image
                else:
                    instance.org_images = new_image

        # Step 3: Handle org_logo update
        org_logo = request.data.get("org_logo")
        if org_logo and org_logo != "null":
            new_logo = save_image_to_azure(
                org_logo,
                instance._id,
                instance.organization_name,
                "hinduworldlogos"
            ...
            if new_logo:
                instance.org_logo = new_logo  # Replace old logo
